...
const tagMeta = (tag: string) => {
  const map: Record<string, { cls: string; icon: string }> = {
    [TAG_URGENT]:     { cls: "bg-red-500/15 text-red-400 border-red-500/25",     icon: "pi-exclamation-triangle" },
    [TAG_SCHEDULE]:   { cls: "bg-blue-500/15 text-blue-400 border-blue-500/25",  icon: "pi-calendar" },
    [TAG_DINING]:     { cls: "bg-amber-500/15 text-amber-400 border-amber-500/25", icon: "pi-star" },
    [TAG_GATHERING]:  { cls: "bg-purple-500/15 text-purple-400 border-purple-500/25", icon: "pi-users" },
    [TAG_ACTIVITY]:   { cls: "bg-emerald-500/15 text-emerald-400 border-emerald-500/25", icon: "pi-bolt" },
    [TAG_HEALTH]:     { cls: "bg-teal-500/15 text-teal-400 border-teal-500/25",  icon: "pi-heart" },
    [TAG_NOTEWORTHY]: { cls: "bg-cyan-500/15 text-cyan-400 border-cyan-500/25",  icon: "pi-info-circle" },
  };
  return map[tag] ?? { cls: "bg-slate-500/15 text-slate-400 border-slate-500/25", icon: "pi-tag" };
};
